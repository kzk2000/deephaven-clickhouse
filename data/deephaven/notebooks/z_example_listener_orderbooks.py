    ...
    for ts, x in zip(ts_array, json_array):
        ...
        my_publisher.add(
            new_table([
                ...
                InputColumn(
                    name="price",
                    data_type=dht.float64_array,
                    input_data=[bid_prices, ask_prices],
                ),
                InputColumn(
                    name="qty",
                    data_type=dht.float64_array,
                    input_data=[bid_qtys, ask_qtys],
                ),
            ])
        )
...
table = (
    time_table("PT00:00:02").update("json_str=generate_snapshot()").tail(5)
)  # FIXME: this is bad practice and won't scale well
handle = listen(table, listener_function)

my_ring_table = (
    blink_tail_by(my_blink_table, 1, by=["symbol", "side"])
    .ungroup()
    .sort(
        ["ts", "symbol", "price"],
