...
]).drop_columns(['bid', 'ask'])  # uncomment this if you wann
...
def parse_book_curated(book_str: str, max_levels: int = 200):
    ...
    order_sizes_to_execute = [10e3, 100e3, 150e3, 250e3, 500e3, 750e3, 1e6, 2e6, 3e6]  # in quote quantity
...
quotes_l2_bid_curated = tmp.select([ # use this for testing
...
quotes_l2_ask_curated = tmp.select([ # use this for testing
# quotes_l2_curated = orderbooks_sampled.select([
    'ts',
    'ts_bin',
    'exchange',
...
l2_book_curated = merge([quotes_l2_bid_curated, quotes_l2_ask_curated]).sort(['ts_bin', 'symbol', 'order_size'])
l2_book_curated_one_symbol = l2_book_curated.where(["symbol == `BTC-USD`"]).where("abs(order_size) == 100000").tail_by(30, ['side'])

plot_l2_curated = Figure() \
    .axes(plot_style=PlotStyle.STEP) \
    .chart_title(title="BTC-USD: Slippage for $100k") \
    .plot_xy(series_name="100k", t=l2_book_curated_one_symbol, x="ts_bin", y="slippage", by=['side']) \
    .show()

import deephaven.plot.express as dx

fig = dx.scatter(l2_book_curated_one_symbol, x="ts_bin", y="slippage", by=['side'],
...
def fill_price_exact(prices, sizes):
    ...
    for i, fill_index in enumerate(fill_indices):
        if sizes_to_execute[i] <= cum_sizes[-1]:
            cumulative_winsorized = np.minimum(cum_sizes[:(fill_index + 1)], sizes_to_execute[i])
            sizes_winsorized = cumulative_winsorized[1:] - cumulative_winsorized[:-1]  # equivalent to np.diff()
            avg_fill_prices[i] = np.dot(prices[:fill_index], sizes_winsorized) / sizes_to_execute[i]
        else:
            break
...
quotes_l2_curated = orderbooks_sampled.tail(3).snapshot().select([ # use this for testing
# quotes_l2_curated = orderbooks_sampled.select([
    'ts',
    'ts_bin',
    'exchange',
